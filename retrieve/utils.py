...
def retrieve_topk(model, topk, descriptions, snippets, embeddings, current_description, device='cuda'):
    current_embedding = model.encode([current_description], device=device, convert_to_tensor=True)
...
def extract_scenic_code(text):
    ...
    raise ValueError(f"No fenced Scenic code block found in LLM response:\n{text}")

def render_spawn_code(spawn_code, adv_object):
    """Render known placeholders without treating Scenic braces as Python format fields."""
    if spawn_code is None:
        raise ValueError("Spawn snippet generation returned None.")

    rendered = spawn_code.replace('{AdvObject}', adv_object)

    opt_string_params = dict(
        re.findall(r"param\s+(OPT_[A-Z0-9_]+)\s*=\s*['\"]([A-Za-z_][A-Za-z0-9_]*)['\"]", rendered)
    )

    def replace_opt(match):
        name = match.group(1)
        return opt_string_params.get(name, f'globalParameters.{name}')

    return re.sub(r'\{(OPT_[A-Z0-9_]+)\}', replace_opt, rendered)
...
def save_scenic_code(local_path, port_ip, scenic_code, q, skip_compile=False):
    ...
    if skip_compile:
        print(f"Scenic code saved at {file_path}; skipped CARLA/Scenic compile check.")
        return True
